...
/** Compute all statuses from a RoadmapConfig (task + all team statuses + "None"). */
export function getAllStatusesFromConfig(config: RoadmapConfig): string[] {
  const teamStatuses = config.teamRoles.flatMap(r => config.teamStatuses[r] || []);
  return ['None', ...config.taskStatuses, ...teamStatuses];
}
